<style>
    ...
    .tab-pane hr {
        border: 0;
        height: 1px;
        background: #333 linear-gradient(to right, #fff2db, #5f4d41, #fff2db);
    }
        ...
        <div class="tab-content" id="nav-tabContent">
            ...
            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                ...
                foreach ($topPlayers as $player) {
                    ...
                    $count = $count + 1;
                    ?>
                    <div class="tplevellayout bgoutfit_<?= $count ?>">
                        <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player['name'], false) ?>">
                            <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                            <span class="firstlevel" style="background: transparent url(<?= $template_path ?>/images/themeboxes/highscores/rank<?= $count ?>.png) no-repeat center;"></span>
                            <div class="rank_text">
                                <span><?= $count ?></span> - <?= $player['name'] ?> <br>
                                <small>Level: (<?= $player['level'] ?>)<br><?= $player_voc ?></small>
                            </div>
                        </a>
                    </div>
                    <hr>
            ...
            <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                ...
                if ($fetch_from_db) {
                    ...
                    foreach ($players_db as $player) {
                        $players[] = array('skulltime'  => $player['skulltime'],
                                           'name'       => $player['name'],
                                           'level'      => $player['level'],
                    ...
                    foreach ($players as $player) {
                        ...
                        <div class="tplevellayout bgoutfit_<?= $count ?>">
                            <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player['name'], false) ?>">
                                ...
                                <div class="rank_text">
                                    <span><?= $count ?></span> - <?= $player['name'] ?> <br>
                                    <small>Frags: (<?= $player['skulltime'] ?>)<br><?= $player_voc ?></small>
                                </div>
                            </a>
                        </div>
                        <hr>
            ...
            <div class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab">
                ...
                <div class="tplevellayout">
                    <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player_ml['name'], false) ?>">
                        <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                        <div class="rank_text" style="margin-left: 45px;">
                            <span>1</span> - <?= $player_ml['name'] ?> <br>
                            <small>Magic Level: (<?= $player_ml['maglevel'] ?>)<br><?= $voc_ml ?></small>
                        </div>
                    </a>
                </div>
                <hr>
                <div class="tplevellayout">
                    <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player_sword['name'], false) ?>">
                        <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                        <div class="rank_text" style="margin-left: 45px;">
                            <span>1</span> - <?= $player_sword['name'] ?> <br>
                            <small>Sword: (<?= $player_sword['skill_sword'] ?>)<br><?= $voc_sword ?></small>
                        </div>
                    </a>
                </div>
                <hr>
                <div class="tplevellayout">
                    <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player_axe['name'], false) ?>">
                        <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                        <div class="rank_text" style="margin-left: 45px;">
                            <span>1</span> - <?= $player_axe['name'] ?> <br>
                            <small>Axe: (<?= $player_axe['skill_axe'] ?>)<br><?= $voc_axe ?></small>
                        </div>
                    </a>
                </div>
                <hr>
                <div class="tplevellayout">
                    <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player_club['name'], false) ?>">
                        <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                        <div class="rank_text" style="margin-left: 45px;">
                            <span>1</span> - <?= $player_club['name'] ?> <br>
                            <small>Club: (<?= $player_club['skill_club'] ?>)<br><?= $voc_club ?></small>
                        </div>
                    </a>
                </div>
                <hr>
                <div class="tplevellayout">
                    <a class="topleveltext top_online" style="text-decoration:none" href="<?= getPlayerLink($player_dist['name'], false) ?>">
                        <div class="outfitImgtoplevel" style="background: url(<?= $player['outfit'] ?>) no-repeat 0 0;"></div>
                        <div class="rank_text" style="margin-left: 45px;">
                            <span>1</span> - <?= $player_dist['name'] ?> <br>
                            <small>Distance: (<?= $player_dist['skill_dist'] ?>)<br><?= $voc_dist ?></small>
                        </div>
                    </a>
                </div>
                <hr>
